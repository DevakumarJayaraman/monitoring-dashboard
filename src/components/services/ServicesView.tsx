...
export function ServicesView(): JSX.Element {
  ...
  const hasGlobalSelection = totalSelectedInstances > 0;

  const handleGlobalStart = () => {
    // Get all selected instances across all services - only stopped ones
    const instances = Object.entries(selectedInstances).flatMap(([serviceKey, instanceIds]) => {
      const service = filteredServices.find(s => `${s.profile}-${s.name}` === serviceKey);
      return service ? service.instances.filter(instance => 
        instanceIds.includes(instance.id) && normaliseStatus(instance) !== "running"
      ) : [];
    });
    
    if (instances.length === 0) {
      console.log('No stopped instances selected to start');
      return;
    }
    
    const serviceNames = Array.from(new Set(instances.map(inst => 
      Object.keys(selectedInstances).find(key => selectedInstances[key].includes(inst.id))?.split('-').slice(1).join('-') || ''
    )));
  ...
  const handleGlobalStop = () => {
    // Get all selected instances across all services - only running ones
    const instances = Object.entries(selectedInstances).flatMap(([serviceKey, instanceIds]) => {
      const service = filteredServices.find(s => `${s.profile}-${s.name}` === serviceKey);
      return service ? service.instances.filter(instance => 
        instanceIds.includes(instance.id) && normaliseStatus(instance) === "running"
    ...
    if (instances.length === 0) {
      console.log('No running instances selected to stop');
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Selected Instances Buttons */}
          {hasGlobalSelection ? (
            <>
              <button
                type="button"
                className="inline-flex items-center gap-1 rounded-full border border-emerald-400/50 px-3 py-1.5 text-sm font-medium text-emerald-200 transition hover:bg-emerald-400/10"
                onClick={handleGlobalStart}
              >
                ▶ Start Selected ({totalSelectedInstances})
              </button>
              <button
                type="button" 
                className="inline-flex items-center gap-1 rounded-full border border-rose-400/50 px-3 py-1.5 text-sm font-medium text-rose-200 transition hover:bg-rose-400/10"
                onClick={handleGlobalStop}
              >
                ■ Stop Selected ({totalSelectedInstances})
              </button>
            </>
          ) : (
            <>
              <button
                type="button"
                className="inline-flex items-center gap-1 rounded-full border border-slate-700 px-3 py-1.5 text-sm font-medium text-slate-400 transition cursor-not-allowed"
                disabled
              >
                ▶ Start Selected (0)
              </button>
                    ...
                    {hasSelection ? (
                      ...
                      isExpanded ? (
                        <span className="text-slate-500">
                          Select instances for bulk actions
                        </span>
                      ) : null
                    )}
                  </div>
                    ...
                    <div className="space-y-3">
                      {service.instances.map((instance) => {
                        ...
                        return (
                          <div
                            key={instance.id}
                            className={`space-y-3 rounded-lg bg-slate-900/60 p-4 shadow-inner transition ${
                              isSelected
                                ? "border border-emerald-400/60 ring-2 ring-emerald-400/30"
                                : "border border-slate-800 hover:border-emerald-300/40"
                            }`}
                            role="checkbox"
                          ...
                          >
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-100">
                                <span className="rounded-full border border-slate-600 bg-slate-800/80 px-2 py-0.5 text-xs font-semibold text-slate-200">
                                  v{instance.version}
                                </span>
                                <span className="rounded-full border border-emerald-400/60 bg-emerald-500/10 px-2 py-0.5 text-xs font-semibold text-emerald-200">
                            ...
                            <div className="text-xs text-slate-400">
                              Host: {instance.machineName} · Port {instance.Port}
                            </div>
                            <div className="text-xs text-slate-400">
                              Deployment Time: {machineMeta?.datacenter ?? "—"}
                            </div>
